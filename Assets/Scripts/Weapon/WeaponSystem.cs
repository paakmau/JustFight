...
namespace JustFight.Weapon {
    ...
    class WeaponSystem : SystemBase {
        ...
        protected override void OnUpdate () {
            ...
            var tankGunJobHandle = Entities.ForEach ((Entity entity, int entityInQueryIndex, in TankHullToFollow hullToFollow, in TankTurretTeam team, in AimInput shootInput, in WeaponState weaponState, in TankGun gun, in LocalToWorld localToWorld) => {
                if (weaponState.isShootTrigger) {
                    ...
                    tankGunJobEcb.SetComponent (entityInQueryIndex, bulletEntity, new BulletTeam { hull = hullToFollow.entity, id = team.id });
                }
            }).ScheduleParallel (Dependency);

            // 双管坦克炮
            var doubleTankGunJobEcb = m_entityCommandBufferSystem.CreateCommandBuffer ().ToConcurrent ();
            var doubleTankGunJobHandle = Entities.ForEach ((Entity entity, int entityInQueryIndex, in TankHullToFollow hullToFollow, in TankTurretTeam team, in AimInput shootInput, in WeaponState weaponState, in DoubleTankGun gun, in LocalToWorld localToWorld) => {
                if (weaponState.isShootTrigger) {
                    ...
                    doubleTankGunJobEcb.SetComponent (entityInQueryIndex, bulletEntity, new BulletTeam { hull = hullToFollow.entity, id = team.id });
            ...
            var shotgunJobHandle = Entities.ForEach ((Entity entity, int entityInQueryIndex, in TankHullToFollow hullToFollow, in TankTurretTeam team, in AimInput shootInput, in WeaponState weaponState, in Shotgun gun, in LocalToWorld localToWorld) => {
                if (weaponState.isShootTrigger) {
                    ...
                    for (int i = 0; i < gun.bulletNum; i++) {
                        ...
                        shotgunJobEcb.SetComponent (entityInQueryIndex, bulletEntity, new BulletTeam { hull = hullToFollow.entity, id = team.id });
                    }
                }
            }).ScheduleParallel (Dependency);
